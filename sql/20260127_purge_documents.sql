--
-- Procedure: sp_purge_documents
-- Removes every row from preventivi/lavorazioni/attivita/ddt/fatture tables and their direct children.
-- Uses information_schema to skip tables that do not exist in the current schema so it is safe
-- to run during migrations or across environments that do not yet own all the cruft.
--

DELIMITER ;;
DROP PROCEDURE IF EXISTS `sp_purge_documents`;
