...
final class CrmSmtpMailer
{
    ...
    public function __construct(
        ...
        $this->host = $host ?? (getenv('CRM_SMTP_HOST') ?: (getenv('SMTP_HOST') ?: 'localhost'));
        $this->port = $port ?? (int) (getenv('CRM_SMTP_PORT') ?: (getenv('SMTP_PORT') ?: 587));
        $this->username = $username ?? (getenv('CRM_SMTP_USERNAME') ?: (getenv('SMTP_USERNAME') ?: null));
        $this->password = $password ?? (getenv('CRM_SMTP_PASSWORD') ?: (getenv('SMTP_PASSWORD') ?: null));
        $this->encryption = strtolower($encryption ?? (getenv('CRM_SMTP_ENCRYPTION') ?: (getenv('SMTP_ENCRYPTION') ?: 'tls')));
        $this->timeout = $timeout ?? (int) (getenv('CRM_SMTP_TIMEOUT') ?: (getenv('SMTP_TIMEOUT') ?: 30));
        $this->ehloDomain = $ehloDomain ?? (getenv('CRM_SMTP_EHLO_DOMAIN') ?: (getenv('SMTP_EHLO_DOMAIN') ?: (gethostname() ?: 'localhost')));
        $this->allowSelfSigned = $allowSelfSigned ?? ((getenv('CRM_SMTP_ALLOW_SELF_SIGNED') ?: getenv('SMTP_ALLOW_SELF_SIGNED')) === '1');
    }
    ...
    ): bool
    {
        ...
        foreach ($additionalHeaders as $headerName => $headerValue) {
            ...
            $safeValue = trim((string) (preg_replace("/[\r\n]+/", ' ', $value) ?? ''));
